...
- **Summaries/frontmatter by default, bodies on request.** A `list` returns
  `name + summary + status/assignment`, never bodies.
- **A hard 20,000-character budget per MCP response.** Multi-item list/map
  responses page under this budget (cursor-based). When any response is
  truncated — including a single body that exceeds the budget — the truncation
  notice is **prepended to the returned content** and names the **on-disk file
  path** for that content, so the agent can read it directly as a fallback when
  the MCP page size is too small to carry it.

Prepending the notice (rather than only setting a separate structured field) is
deliberate: an in-content notice is impossible to miss. That is the core lesson
from the earlier project — returning "everything" risks *silent* client-side
truncation that neither side can detect.

The file-path fallback is a pragmatic escape hatch. It sits in mild tension with
the long-term goal of the agent having no awareness of `.sourcebook`'s on-disk
layout (see `CLAUDE.md`), but direct `Read` of `.sourcebook` is deliberately not
blocked today, and the fallback only triggers in the exceptional
content-exceeds-budget case.
...
- Stored in **frontmatter** (always), so it is trivially extractable without
  parsing the body.
- **Capped at 500 characters.** Its purpose is narrow: give the agent enough to
  decide *whether it needs to read the full body* — a routing signal, not a
  substitute for the body. The cap is also what makes the primed payload
  *predictable*.
- **Generated by the tool, approved by the GM** (generate-and-approve). At edit
  time the tool drafts the summary from the new body; the GM approves (or edits)
  it before it commits. This keeps a human gate on a governance artifact while
  removing the burden of hand-authoring every summary.
- **When no summary exists**, the read path returns an explicit placeholder
  message stating the summary is absent (rather than an empty field), so the
  agent knows to fall back to reading the body instead of assuming there is
  nothing to know.
...
These were the open questions at first draft; all four are now decided.

1. **How summaries are produced — generate-and-approve.** The tool drafts the
   summary at edit time from the new body; the GM approves (or edits) it before
   it commits. Keeps a human gate on a governance artifact without hand-authoring
   every summary.
2. **Summary length cap — 500 characters.** Enough for the agent to decide
   whether to read the full body; a routing signal, not a body substitute.
3. **Storage — frontmatter field, always.** Guarantees trivial extraction. When
   absent, the read path returns an explicit "no summary" placeholder message.
4. **Response budget & truncation shape — 20,000-character limit.** On
   truncation, the notice is prepended to the returned content and names the
   on-disk file path, so the content can be read directly as a fallback when the
   MCP page size is too small. Multi-item responses page under the same budget.
...
- Separate **deterministic traversal** (aggregate server-side) from
  **relevance-ranked search** (two-call refs→hydrate); apply each only to its
  matching domain.
- Add a **500-character `summary`** frontmatter field to **region and lore**,
  **generated-and-approved** at edit time and materialized on the edit
  transition so it is never stale; return an explicit placeholder when absent.
- Serve a **tiered read**: live selection → precomputed summary → body on demand.
- Enforce a **20,000-character response budget**; on truncation, prepend a notice
  naming the on-disk **file path** for a direct-read fallback.
