...
This repo's own `.claude/skills/` and `.agents/skills/` remain the working source-of-truth copies and are edited directly as normal — they are *not* generated from the packaged templates. An edit to either working copy must be manually re-copied into the corresponding `templates/skills/{claude,codex}/<name>/SKILL.md` file, or the packaged/deployed content will silently drift stale relative to this repo's own skills.
