...
Two functionally-equivalent sets of skills wrap the `cac` MCP server (falling back to the CLI when it's unavailable) so an agent session can drive `.sourcebook` content without ever touching it directly, per the guardrail above: Claude Code originals under `.claude/skills/`, and a Codex-native mirror under `.agents/skills/` (referencing the same MCP tools by their bare registered names, since Codex's MCP invocation surface doesn't share Claude Code's `mcp__cac__`-prefixed form; see `AGENTS.md`).
...
These are currently authored directly under `.claude/skills/` and `.agents/skills/` in this repository. Once they're stable, they will move to `packages/crypts-and-commits/src/cac/core/templates/skills/` (a sibling of `templates/sourcebook/`) and be deployed into a target project's `.claude/skills/`/`.agents/skills/` by `cac bootstrap`, the same way `world.md` is templated into `.sourcebook/world.md` today. Do not start that migration until the skills themselves are fully bootstrapped and working — this is forward-looking context, not a current task.
